...
def find_server(client):
    ...
    instances = []
    response = client.describe_instances(Filter=Filter)

    instances_full_details = response['Reservations']
    for instance_detail in instances_full_details:
        instances += [x['NetworkInterfaces'][0]['Association']['PublicIp'] for x in instance_detail['Instances']]
